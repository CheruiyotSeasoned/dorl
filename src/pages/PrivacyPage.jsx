...
export default function PrivacyPage() {
  ...
  const title   = legal.privacy_title   ?? 'Privacy Policy'
  const updated = legal.privacy_updated ?? 'June 2026'
  const content = legal.privacy_content ?? ''
    ...
    <div style={{ minHeight: '100vh', background: '#fafafa', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ background: '#0D0D0D', padding: '24px', textAlign: 'center' }}>
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', margin: 0 }}>
          ...
          &nbsp;·&nbsp;
          <Link to="/privacy" style={{ color: '#FF5E14', textDecoration: 'none' }}>Privacy</Link>
        </p>
      </div>
    </div>
  )
